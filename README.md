...
| 来源 | 说明 | 更新频率 |
|------|------|----------|
| **EasyList** (YouTube + 关联广告域) | 从 EasyList 完整列表中 grep 提取的 YouTube 专用规则，覆盖 youtube.com / googlevideo.com / ytimg.com / doubleclick.net / googlesyndication.com / googleadservices.com，CSS 隐藏 + 网络拦截 | 每日 |
| **AdGuard Filter 2** (YouTube + 关联广告域) | YouTube JS 脚本注入（set-constant / json-prune / adjust-setTimeout）+ 网络请求拦截 + CSS 隐藏，覆盖域名同上，对抗 YouTube 反广告检测 | 每日 |
...
2. 从 EasyList 和 AdGuard 中提取 YouTube 及其关联广告域（`youtube.com` / `googlevideo.com` / `ytimg.com` / `doubleclick.net` / `googlesyndication.com` / `googleadservices.com`）的规则
3. 合并为单一文件 ublock-rules.txt
4. 提交并推送至本仓库
5. 自动创建当日 Release
...
- **EasyList 太大**：完整 EasyList 有数万条规则，其中 99% 针对英文网站。本项目仅提取 YouTube 相关规则，不会误杀中文网站。同时覆盖 Google 关联广告域名（doubleclick.net / googlesyndication.com / googleadservices.com），不遗漏 YouTube 周边广告生态。
